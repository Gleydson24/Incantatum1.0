...
class ModoTreinamento:
    ...
    def update_logica(self):
        ...
        # --- COLISÃO DE MAGIAS (DUELO) ---
        # Se baterem, ambas somem e cria explosão
        ...
        for m_player, m_inimigos in hits_magia.items():
            ...
            for _ in range(8):
                self.particulas.append(ParticulaMagica(centro[0], centro[1], (255, 200, 50), "explosao"))

        # --- DANO NO INIMIGO (Com Protego) ---
        hits = pygame.sprite.spritecollide(self.inimigo, self.magias_player, False) # False para não deletar antes de checar
        for m in hits:
            if self.inimigo.escudo_ativo:
                ...
                m.kill() # Mata a magia no escudo
                self.inimigo.escudo_ativo = False # Quebra o escudo
            else:
                # Dano normal
