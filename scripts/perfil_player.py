...
class PerfilJogador:
    def __init__(self, jogo):
        ...
        self.personagens_ia = [{"nome": "Harry Potter", "cor": (255,100,100)}, {"nome": "Dumbledore", "cor": (150,150,255)}]
        ...
        self.msg_erro_login = ""
        self.btn_login = Botao(self.x + 300, self.y + 400, 150, 40, "ENTRAR", cor_fundo=(0, 100, 0))
        self.btn_registrar = Botao(self.x + 500, self.y + 400, 150, 40, "CRIAR CONTA", cor_fundo=(0, 0, 100))
        self.btn_refresh = Botao(self.x + 750, self.y + 230, 100, 30, "Atualizar", cor_fundo=(50,50,50), tamanho_fonte=14)
        self.btn_add_friend = [] # Lista de botões dinâmicos
    ...
    def desenhar_aba_chat(self):
        ...
        # Filtra lista para mostrar amigos reais se estiver no modo amigos
        lista_display = []
    ...
    def desenhar_aba_conta(self):
        x, y = self.x + 50, self.y + 250
        if not CLIENTE_ONLINE.conectado:
            self.tela.blit(self.fonte_secao.render("LOGIN / CADASTRO", True, OURO), (x + 350, y))
            c_u = OURO if self.foco_input == "user" else CINZA; c_p = OURO if self.foco_input == "pass" else CINZA
            pygame.draw.rect(self.tela, (30,30,40), (x+250, y+50, 400, 40), border_radius=5); pygame.draw.rect(self.tela, c_u, (x+250, y+50, 400, 40), 2, border_radius=5)
            self.tela.blit(self.fonte_texto.render(self.input_user, True, BRANCO), (x+260, y+60))
            pygame.draw.rect(self.tela, (30,30,40), (x+250, y+110, 400, 40), border_radius=5); pygame.draw.rect(self.tela, c_p, (x+250, y+110, 400, 40), 2, border_radius=5)
            self.tela.blit(self.fonte_texto.render("*"*len(self.input_pass), True, BRANCO), (x+260, y+120))
            self.btn_login.desenhar(self.tela); self.btn_registrar.desenhar(self.tela)
            if self.msg_erro_login: self.tela.blit(self.fonte_mini.render(self.msg_erro_login, True, (255,100,100)), (x+250, y+160))
        else:
            # NOVO LAYOUT DE RANKING
            elo = CLIENTE_ONLINE.elo
            tier = CLIENTE_ONLINE.tier
            cor_tier = CORES_TIER.get(tier, BRANCO)
            
            # Badge Grande do Elo
            pygame.draw.circle(self.tela, (30, 30, 40), (x + 100, y + 80), 70)
            pygame.draw.circle(self.tela, cor_tier, (x + 100, y + 80), 70, 5)
            lb_tier = self.fonte_secao.render(tier.upper(), True, cor_tier)
            self.tela.blit(lb_tier, (x + 100 - lb_tier.get_width()//2, y + 60))
            lb_elo = self.fonte_titulo.render(str(elo), True, BRANCO)
            self.tela.blit(lb_elo, (x + 100 - lb_elo.get_width()//2, y + 90))
            ...
            for i, p in enumerate(CLIENTE_ONLINE.top_rank):
                c = OURO if i == 0 else (BRANCO if i < 3 else CINZA)
                t_tier = CORES_TIER.get(p.get('tier'), BRANCO)
                ...
                y_list += 28
                if i >= 8: break # Mostra só top 8 pra caber
    ...
    def processar_eventos(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.aba_ativa == "conta" and not CLIENTE_ONLINE.conectado:
                x, y = self.x + 50, self.y + 250
                r_u, r_p = pygame.Rect(x+250, y+50, 400, 40), pygame.Rect(x+250, y+110, 400, 40)
                ...
                if self.btn_login.desenhar(self.tela):
                    if not self.input_user.strip(): self.msg_erro_login = "Digite um usuário!"
                        ...
                        else: CLIENTE_ONLINE.atualizar_dados()
